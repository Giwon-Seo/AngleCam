...
(function strict() {
    'use strict';
    var DELAY_0_SECOND=0, DELAY_2_SECOND = 2, DELAY_5_SECOND = 5, DELAY_10_SECOND = 10, A = 1;
    ...
    SelfCamera.prototype.setTimer = function setTimer(value) {
        this.timer = value;
        // clear buttons
        $('#timer0, #timer2, #timer5, #timer10').removeClass('selected');
        // mark active button
    ...
    SelfCamera.prototype.onCaptureVideoSuccess =
        function onCaptureVideoSuccess(stream) {
            ...
            // this.setTimer(DELAY_0_SECOND);
            $('#timer0').addClass('selected'); //Timer0버튼 Active 
        };
    ...
    SelfCamera.prototype.savePosition = function savePosition(){
    	deleteFile(); /** after delete the file, create the file to read*/
    };
    /**
    ...
    SelfCamera.prototype.shutterTouched = function shutterTouched() {
        ...
        if (this.isMediaWorking) {
        	if(A==1){
        		...
        		this.startCountdown();// start count down   
        	}
    ...
    SelfCamera.prototype.bindTimerClicks = function bindTimerClicks() {
    	$('#timer0').on('click', function(e){
    		A=1;//Time0==>set
    		$('#timer0, #timer2, #timer5, #timer10').removeClass('selected');//button remove
    		$('#timer0').addClass('selected');//버튼Active
    	});
...
var CHEADER_HEIGHT = 50;
var CscreenWidth;
var CscreenHeight;
var XcenterX;
var XcenterY;

var YHEADER_HEIGHT = 110;
var YcenterX;
var YcenterY;

/** Define Favorite Angle*/
var pos = new Array(2); // Postion( for degree)
function Pos(){
...
pos[0] = new Pos(); // pos[0] -> 좌측 각도
pos[1] = new Pos(); // pos[1] -> 정면 각도


var Yicon_WIDTH = 20; /** px **/
var Xicon_WIDTH = 20; /** px **/

var Xbar_HEIGHT = 20;/** px **/  
var Ybar_HEIGHT = 360;/** px **/
var Ybar_WIDTH = 20;/** px **/

var xPos;
var yPos;
var zPos;

function ConOrientationChange(dataEvent) {
	...
	/*information.innerHTML = "xPos" + Math.round(xPos) + "<br>" +
							"yPos" + Math.round(yPos) + "<br>" +
							"zPos" + Math.round(zPos) ;*/
	if(yPos < 0 ){
		circleX.classList.add('reserved');
		circleY.classList.add('reserved');
	}
	else {
		circleX.classList.remove('reserved');
		circleY.classList.remove('reserved');
	}
	
	// side
	circleX.style.top = XcenterY + "px";
	circleX.style.left = XcenterX + 
						 Math.round( ( Math.atan(-xPos/ yPos)/Math.PI )*(320/360) * 180 ) * 2 + "px";
	
	// front
	circleY.style.top =  YcenterY + Math.round( ( Math.atan(zPos/-yPos) / Math.PI ) * 180 ) * 2 + "px";
					//	 Math.round( ( Math.atan( Math.sqrt( Math.pow(zPos,2)+ Math.pow(xPos,2) ) /-yPos)/Math.PI ) * 180 ) * 2 + "px";
	circleY.style.left = YcenterX + "px";
	
	
	// Math.pow(,2), Math.sqrt()
	pos[0].top  = XcenterY;
	pos[0].left = XcenterX + Math.round( ( Math.atan(-xPos/ yPos)/Math.PI ) *180 )*2*(320/360);
		
	pos[1].top  = YcenterY + Math.round( ( Math.atan(zPos/-yPos)/Math.PI ) *180) *2;
	pos[1].left = YcenterX;
	
}
function CsetDefaultVariables() {
	CscreenWidth = document.width;
	CscreenHeight = document.height - CHEADER_HEIGHT;
	
	XcenterX = CscreenWidth/2 -Xicon_WIDTH/2;
	XcenterY = CHEADER_HEIGHT + Xbar_HEIGHT;
	
	YcenterX = CscreenWidth - Ybar_WIDTH - Yicon_WIDTH;
	YcenterY = Ybar_HEIGHT/2 + YHEADER_HEIGHT - 10;
	
	if (CscreenWidth <= 0 || CscreenHeight <= 0) {
	    return false;
	}
	
	return true;
}
function CsetDefaultViews() {
	var circleX = document.getElementById("circleX"),
		errorMessage = document.getElementById("CerrorMessage"),
		circleY = document.getElementById("circleY");
	
	
	circleX.style.left = XcenterX  + "px";
	circleX.style.top = XcenterY + "px";
	
	circleY.style.left = XcenterX  + "px";
	circleY.style.top = XcenterY + "px";
	
	circleY.style.display = "block";
	circleX.style.display = "block";
	errorMessage.style.display = "none";
}
function CkeyEventCB(event) {
...
function CsetDefaultEvents() {
	document.addEventListener("tizenhwkey", CkeyEventCB);
	
	if (window.DeviceMotionEvent) {
	    window.addEventListener("devicemotion", ConOrientationChange);
	   // add//
	} else {
	    return false;
	}
	...
	return true;
	} // end of CsetDefaultEvents()
	
function setError(text) {
	var CerrorMessage = CemptyElement(document.querySelector("#errorMessage"));
	
	CerrorMessage.appendChild(document.createTextNode(text));
}

//Initialize function
function Cinit() {
		
	if (CsetDefaultEvents() && CsetDefaultVariables() ) {
	    CsetDefaultViews();
	} else {
...
var onCameraSuccess = function(imgData){
	$("#imgCamera").attr("src", "data:image/jpeg;base64," + imgData);       
};
var onCameraError = function(message){
    alert(message);
};
//Initialize function
var init = function () {
...
function Sensing() {
	  ...
	  FAngle = Math.round( ( Math.atan(z/-y)/Math.PI )*180 );
	  SAngle = Math.round( ( Math.atan(-x/y)/Math.PI )*180 );
	  AngleInfo.innerHTML = 
		  	"Front Angle :" + FAngle + "<br>"
	      + "Side  Angle :" + SAngle;	  
	  }, true);
}
/** Define variable */
var AngleDir = 'images/AngleCam';

/** createFile */
function createFile(){
	
	/*var testfunc = document.getElementById("testfunc");*/
	...
	function onsuccess(files) {
		var testFile;
			...
			testFile = documentsDir.createFile("Angle["+ files.length +"].txt");
			/*testfunc.innerHTML = "Success createFile<br>"+"File Name is " + "Angle["+files.length+"].txt";    
	  */
			// file write
			if (testFile !== null) {
				testFile.openStream(
	...
	function onerror(error) {
		/*testfunc.innerHTML = "The error " + error.message + 
...
function readAsText(){
	...
	var	RcircleX = document.getElementById("RcircleX");
	var	RcircleY = document.getElementById("RcircleY");
	var documentsDir;
	var VisibliityStar = RcircleX.style.visibility;
	// var info = document.getElementById("infomation");
	
	/* 보이기/감추기  */
	
	 if(VisibliityStar === "visible"){
		 	RcircleX.style.visibility = "hidden";
	    	RcircleY.style.visibility = "hidden";
    	}
	 else{
		 	RcircleX.style.visibility = "visible";
	    	RcircleY.style.visibility = "visible";	 
	 }
    //	info.innerHTML = VisibliityStar;
	...
	function onsuccess(files) {
			  ...
			  if (files[files.length-1].isDirectory === false){ // files[0] is file ( Not a Directory )
		    	 files[files.length-1].readAsText(
		           function(str) {

		        	var arr = str.split(" "); // " " 로 str 나눠서 arr에 저장
		            
		            
		        	
		            RcircleX.style.left = parseInt(arr[0])+"px";
		           	RcircleY.style.top = parseInt(arr[1])+"px";		

		            RcircleX.style.top = XcenterY + "px";		     
		        	RcircleY.style.left = YcenterX + "px";
		        	
		        	if(arr[2] < 0 ){
		        		RcircleX.classList.add('reserved');
		        		RcircleY.classList.add('reserved');
		        	}
		        	else{
		        		RcircleX.classList.remove('reserved');
		        		RcircleY.classList.remove('reserved');
		        	}
		        	}, function(e) {
		             console.log("Error " + e.message);
		           	 }, "UTF-8"
	 ...
	 function onerror(error) {
		 //testfunc.innerHTML = "The error " + error.message + " occurred when listing the files in the selected folder";
	 }	 
}
/** deleteFile */
function deleteFile(){
		     ...
		     if (!files[i].isDirectory) {
		       documentsDir.deleteFile(
		           ...
		           function() {
		        	  ...
		        	  // testfunc.innerHTML.log("Error" + e.message);
		           });
		     }
		   }
		   //testfunc.innerHTML = "deleteFile";
		   createFile(); /** create the file to read */
		 }
